...
export default {
  ...
  async mounted() {
    await this.fetchVods()
  },
  methods: {
    async fetchVods() {
      try {
        const vods = await this.$axios.get(`${this.$config.apiURL}/v1/vods/all`)
        this.rows = vods.data
      } catch (error) {
    ...
    async onCellClick(params) {
      if (params.column.field === 'edit') {
        this.$router.push({ path: `/admin/vods/${params.row.id}` })
      }
      if (params.column.field === 'delete') {
        if (
          confirm(
            `Are you sure you want to permanently delete VOD ${params.row.id} and all of it's data?`
          )
        ) {
          try {
            await this.$axios.delete(
              `${this.$config.apiURL}/v1/vods/${params.row.id}`
            )
            this.$toast.success('VOD deleted')
            this.fetchVods()
          } catch (error) {
            console.log(error)
            this.$toast.error('Error deleting vod')
          }
        }
      }
    },
