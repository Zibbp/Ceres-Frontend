...
export default {
  ...
  methods: {
    async fetchUsers() {
      try {
        const users = await this.$axios.get(`${this.$config.apiURL}/v1/users`)
    ...
    async onCellClick(params) {
      if (params.column.field === 'edit') {
        this.$router.push({ path: `/admin/users/${params.row.username}` })
      }
      if (params.column.field === 'delete') {
        if (
          confirm(
            `Are you sure you want to delete user ${params.row.username}?`
          )
        ) {
          try {
            await this.$axios.delete(
              `${this.$config.apiURL}/v1/users/${params.row.id}`
            )
